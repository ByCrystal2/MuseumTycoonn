    ...
    public void AcceptButton()
    {
        ...
        if (purchasedRoom.CurrentShoppingType == ShoppingType.Gem)
        {
            ...
            else
            {
                ...
                Debug.Log(purchasedRoom.availableRoomCell.CellLetter + purchasedRoom.availableRoomCell.CellNumber + "  Numaral� Oday� Sat�n Almaya Paran Yetmedi.");
            }
        }
    ...
    private bool TryGetRoomData(int roomId, out RoomData room, List<RoomData> _roomDatas, out string _exMessage)
    {
        ...
        if (room != null)
        {
            ...
            if (multipleRooms)
            {
                ...
                _exMessage =  room.ID + " Bu ID'den Birden fazla oda mevcut. Bu odalar �unlard�r: ";
                ...
                // B�t�n odalar�m�z�n bulundu�u roomdatas listesinde, mevcut odam�z�n id'si ile e�le�en odalar�n RoomCell'ini listeledik ve RoomCell tipinde ki roomCodes adl� Listeye aktard�k.          
                
                
                foreach (RoomCell roomCode in roomCodes) // �rn: 3 adet roomCode var.
                {
                    
                    _exMessage += (" " + roomCode.CellLetter + roomCode.CellNumber + " Kodlu Oda.").ToString(); //  room.ID + " Bu ID'den Birden fazla oda mevcut. Bu odalar �unlard�r: A1 Kodlu Oda. A2 Kodlu Oda. A3 Kodlu Oda."
                }
        ...
        {
            // Belirtilen ID'ye sahip oda bulunamad�
            _exMessage = "Belirtilen ID'ye sahip oda bulunamad�";
            return false;
        }
    }
...
public partial class RoomManager // Room Bonus Controller
{
    ...
    public IEnumerator UpdateAndActivatePictureElement(RoomData _room, PictureElement pe, bool activateRoom)
    {
        ...
        // Coroutine'in tamamlanmas�n� bekleyin
