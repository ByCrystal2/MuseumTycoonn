...
public class GameManager : MonoBehaviour
{
    ...
    {
        // 1. Ad�m: T�m metinleri birle�tir
        string combinedText = string.Join("|", textsToTranslate);

        // 2. Ad�m: Birle�ik metni �evir
        string translatedText = await translation.TranslateTextAsync(combinedText, GetLanguageShortString(GetGameLanguage()));

        // 3. Ad�m: �evirilen metni par�alara ay�r
        List<string> translatedParts = new List<string>(translatedText.Split('|'));

        // �eviri tamamland���nda callback fonksiyonunu �a��r�n
        onTranslationComplete?.Invoke(translatedParts);
    }
    public async System.Threading.Tasks.Task BulkTranslateAndAssignAsync(string language,List<LanguageData> textsToTranslateDatas,Action<List<LanguageData>> onTranslationComplete) // D�N�� T�P�N� List<LanguageData> YAPTIK!
    {
        // 1. Ad�m: �evrilecek metinleri ve TargetID'leri e�le�tiren bir liste olu�tur
        List<(int TargetID, string Key)> textsWithIds = textsToTranslateDatas
            .Select(data => (data.TargetID, data.Key))
            .ToList();

        // �evrilecek metinleri al
        List<string> textsToTranslate = textsWithIds.Select(x => x.Key).ToList();

        // 2. Ad�m: Metinleri birle�tir
        string combinedText = string.Join("|", textsToTranslate);

        // 3. Ad�m: Birle�ik metni �evir
        string translatedText = await translation.TranslateTextAsync(combinedText, GetLanguageShortString(language));

        // 4. Ad�m: �evirilen metni par�ala
        List<string> translatedParts = new List<string>(translatedText.Split('|'));

        // 5. Ad�m: �evirilen metinleri tekrar TargetID ile e�le�tir
        ...
        // 6. Ad�m: �eviri tamamland���nda callback fonksiyonunu �a��r
    ...
    public async System.Threading.Tasks.Task LoadRooms()
    {
        ...
        if (!IsFirstGame)
        {
            ...
            catch (System.Threading.Tasks.TaskCanceledException)
            {
                ...
                return; // ��lem iptal edildi�inde geri d�n.
            }
            catch (Exception ex)
            {
                Debug.LogError("Error loading game data: " + ex.Message);
                return; // Hata durumunda geri d�n.
            }
        }
    ...
    private void CancelFirebaseOperations()
    {
        if (cts != null)
        {
            cts.Cancel();
            cts.Dispose();
            cts = new System.Threading.CancellationTokenSource(); // Yeni bir token olu�tur.
        }
    }
...
namespace TaskExtensions
{
    ...
    public static class TaskHelper
    {
        public static async Task<T> WithCancellation<T>(this Task<T> task, CancellationToken cancellationToken)
        {
            ...
            return await task; // task ba�ar�yla tamamland�, sonucu d�n.
        }
    }
}
...
namespace Translator
{
    ...
    public class DynamicTranslation : MonoBehaviour
    {
        ...
        {
            // I2 Localization kullanarak �eviri i�lemi
            // ForceTranslate fonksiyonunu do�rudan �a��r�yoruz.
            string localizedText = GoogleTranslation.ForceTranslate(text, "auto", targetLanguage);

            Debug.Log("LocalizedText => " + localizedText);

            // �evirinin ba�ar�l� olup olmad���n� kontrol edin
            ...
            {
                Debug.LogError("�eviri ba�ar�s�z oldu.");
                return text; // �eviri ba�ar�s�zsa, orijinal metni d�nd�r
            }
        }
    }
}